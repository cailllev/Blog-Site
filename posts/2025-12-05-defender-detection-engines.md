...
## My Observations
Below is my theory how Defender (or any modern EDR) catches your malware.<br>
A red "m" means the executable is detected.<br><br>

![Defender Scanning Engines](/static/defender-detection-engines/defender-scanning-engines.png)

### Storing on Disk (pre execution)
1) The heuristics engine tries to match your executable to known bad signatures, like hashes, imphashes, yara rules, etc.<br>
2) The emulation engine emulates the file and after a certain cutoff (may be from 0.1sec to 3sec) dumps the memory.<br>
3) The memory scanning engine reads the dumped memory and again tries to match it to known bad signatures.<br>

### Start Execution
4) The EDR may repeat steps 1-3, or use cached results before starting the actual executable.<br>
...
6) When suspicious behaviour is encountered, the executable is either directly marked as malware, or another memory scan is conducted.<br>
...
- Do some deconditioning, so that your future malicious actions are no longer tracked or your memory is scanned. See [Decondition Everything](/post/2025-12-05-decondition-everything)
